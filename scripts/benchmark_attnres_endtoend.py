...
def test_attnres_basic_generation():
    ...
    if not match:
        diff_positions = (output_no_attnres[0, prompt_len:] != output_with_attnres[0, prompt_len:]).nonzero(as_tuple=True)[0]
...
def test_attnres_block_structure():
    ...
    with torch.no_grad():
        ...
        for i, attnres in enumerate(model.attnres_modules):
            print(f"  Layer {i}: attn_q={attnres.pseudo_query_attn.shape}, mlp_q={attnres.pseudo_query_mlp.shape}")
...
def test_attnres_pseudo_query_learning():
    ...
    all_zeros = True
    for i, attnres in enumerate(model.attnres_modules):
        attn_q_zero = torch.allclose(attnres.pseudo_query_attn, torch.zeros_like(attnres.pseudo_query_attn))
        mlp_q_zero = torch.allclose(attnres.pseudo_query_mlp, torch.zeros_like(attnres.pseudo_query_mlp))
...
def test_attnres_long_sequence():
    ...
    for seq_len in seq_lengths:
        ...
        print(f"  Seq len {seq_len:3d}: AttnRes={t_attnres:.3f}s, No AttnRes={t_no_attnres:.3f}s, "
              f"overhead={t_attnres/t_no_attnres:.2f}x")
        
        results.append({
            'seq_len': seq_len,
            'attnres_time': t_attnres,
            'no_attnres_time': t_no_attnres
        })
