...
from src.rabitq import (
    RaBitQ, create_k1, create_k2, create_k3,
    ...
    for dist_name, data in test_cases:
        ...
        for config_name, total_bits in configs:
            ...
            orig_sim = torch.nn.functional.cosine_similarity(
                sample_data[:-1], sample_data[1:], dim=-1
            ).mean().item()
            recon_sim = torch.nn.functional.cosine_similarity(
                reconstructed[:-1], reconstructed[1:], dim=-1
            ).mean().item()
            sim_preserved = abs(orig_sim - recon_sim) < 0.1
            
            print(f"  {config_name}: MAE={mae:.4f}, Max={max_err:.4f}, "
...
def main():
    ...
    if 'kv_pipeline' in all_results and all_results['kv_pipeline']:
        for r in all_results['kv_pipeline']:
            print(f"• {r['name']}: {r['compression_ratio']:.1f}x compression, "
