    ...
    def evaluate_single(self, model, context_length: int, depth_percent: float = 50) -> float:
        ...
        with torch.no_grad():
            try:
                ...
                pred_positions = range(max(0, needle_position - 5), min(context_length - 1, needle_position + 5))
...
class ModelBuilder:
    ...
    def build_model(self) -> AdaptiveTransformer:
        ...
        print(f"   - AttnRes params: {attnres_params / 1e6:.2f}M ({attnres_params/actual_params*100:.3f}%)")
    ...
    def run_efficiency_benchmarks(self, model: AdaptiveTransformer):
        ...
        for seq_len in seq_lengths:
            ...
            try:
                ...
                print(f"      Time: {avg_time*1000:.1f}ms, Throughput: {tokens_per_sec:.1f} tokens/sec")
        ...
        if seq_tests:
            ...
            for test in seq_tests:
                if "error" not in test:
                    report.append(f"  - {test['seq_len']} tokens: {test['avg_time_ms']:.1f}ms ({test['tokens_per_sec']:.1f} tok/s)")
