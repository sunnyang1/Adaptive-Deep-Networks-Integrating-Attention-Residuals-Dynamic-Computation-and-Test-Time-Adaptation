...
def validate_model(model_size):
    ...
    context_lengths = [1024, 4096, 16384, 32768] if model_size == 'small' else [4096, 16384, 32768, 65536]
...
def main():
    ...
    for model_size, results in all_results.items():
        ...
        param_str = f"{config['params'] / 1e9:.1f}B" if config['params'] > 1e9 else f"{config['params'] / 1e6:.1f}M"
