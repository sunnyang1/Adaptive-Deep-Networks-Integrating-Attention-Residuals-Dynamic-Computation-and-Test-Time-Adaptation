...
from src.rabitq import (
    RaBitQ, RaBitQConfig, create_k1, create_k2, create_k3,
    quantize_vector, RabitqConfig, reconstruct_vector,
    RaBitQCache
)
...
def run_compression_benchmark(
    ...
    max_abs_error = max(
        (keys - keys_dq).abs().max().item(),
        (values - values_dq).abs().max().item()
    )
    mean_abs_error = ((keys - keys_dq).abs().mean().item() + 
                      (values - values_dq).abs().mean().item()) / 2
    
    keys_norm = keys.norm().item()
    relative_error = ((keys - keys_dq).norm().item() + 
                      (values - values_dq).norm().item()) / (2 * keys_norm)
...
def benchmark_standalone_compression():
    ...
    for seq_len in seq_lengths:
        ...
        for config_name, factory in configs:
            ...
            print(f"    Quality: max_error={result.max_abs_error:.4f}, "
                  f"rel_error={result.relative_error:.2%}")
            print(f"    Speed: compress={result.compress_time_ms:.1f}ms, "
...
def benchmark_transformer_integration():
    ...
    for seq_len in seq_lengths:
        ...
        print(f"  RaBitQ 1-bit forward: {rabitq_time:.1f}ms "
        ...
        t0 = time.perf_counter()
        with torch.no_grad():
            output = model.generate(prompt_ids, max_new_tokens=max_new_tokens, 
                                    use_rabitq=True)
        t1 = time.perf_counter()
        gen_time = t1 - t0
        
        tokens_per_sec = max_new_tokens / gen_time
        print(f"  Generation: {tokens_per_sec:.1f} tokens/sec "
...
def print_summary_table(results: List[BenchmarkResult]):
    ...
    print("SUMMARY TABLE")
    print("=" * 80)
    print(f"{'Config':<15} {'SeqLen':<8} {'Ratio':<8} {'MaxErr':<8} "
          f"{'Cmp(ms)':<8} {'Dcmp(ms)':<8} {'Thrput':<10}")
    print("-" * 80)
    
    for r in results:
        print(f"{r.config_name:<15} {r.seq_len:<8} {r.compression_ratio:<8.1f}x "
