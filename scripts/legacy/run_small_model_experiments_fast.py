...
def run_experiments():
    ...
    for mem in kv_memory:
        print(
            f"  {mem['seq_len']:5d} tokens: Model={mem['model_mb']:.1f}MB, "
    ...
    print(
        f"  Status: {'Negligible (<0.1%)' if attnres_params/total_params < 0.001 else 'Low (<1%)'}"
    )
    ...
    for arch, metrics in architectures.items():
        print(
            f"{arch:<15} {metrics['attenuation']:<12.2f} {metrics['effective_depth']:<12d} {metrics['cv']:<8.2f}"
        )
...
def generate_report(results: Dict) -> str:
    ...
    for name, metrics in arch.items():
        report.append(
            f"{name:<15} {metrics['attenuation']:<12.2f} "
