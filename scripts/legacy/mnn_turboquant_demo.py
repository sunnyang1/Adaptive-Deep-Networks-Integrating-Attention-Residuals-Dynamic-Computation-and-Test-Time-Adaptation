...
def demo_quantization():
    ...
    for name, mode in [("FP16", 8), ("KV-TQ4", 14)]:
        ...
        # Calculate sizes
        orig_size = (
            keys.element_size() * keys.nelement() + values.element_size() * values.nelement()
        )
...
def demo_recommendations():
    ...
    for name, params in model_sizes:
        ...
        rec_tq4 = (
            "✅ Recommended"
            if config_tq4.is_recommended_for_model_size(params)
            else "⚠️ Not recommended (<4B)"
        )
        rec_tq3 = (
            "✅ Recommended"
            if config_tq3.is_recommended_for_model_size(params)
            else "⚠️ Not recommended (<4B)"
        )
...
def main():
    ...
    if not any(
        [args.modes, args.compression, args.memory, args.quantization, args.recommendations]
    ):
        args.all = True
