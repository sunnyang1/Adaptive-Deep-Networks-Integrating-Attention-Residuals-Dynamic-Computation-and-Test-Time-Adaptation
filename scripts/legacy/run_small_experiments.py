...
def run_experiments():
    ...
    print(
        f"  AttnRes overhead: {attnres_params / 1e6:.2f}M ({attnres_params/total_params*100:.4f}%)"
    )
    ...
    for r in latency_results:
        if "error" not in r:
            print(
                f'  {r["seq_len"]:4d} tokens: {r["avg_time_ms"]:8.1f}ms ({r["throughput_tok_per_sec"]:6.1f} tok/s)'
            )
