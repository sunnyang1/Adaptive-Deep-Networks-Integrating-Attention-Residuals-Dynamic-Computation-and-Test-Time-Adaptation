...
echo "   显存需求: ~24GB"
echo "   命令: python scripts/train_streaming.py --model-size small --max-steps 10000"
echo ""
echo "2) Medium 模型 (8.7B) - 单卡流式训练"
echo "   显存需求: ~92GB"
echo "   命令: python scripts/train_streaming.py --model-size medium --max-steps 10000"
echo ""
echo "3) Medium 模型 (8.7B) - 4卡流式训练 (推荐!)"
echo "   显存需求: ~92GB per GPU"
echo "   命令: torchrun --nproc_per_node=4 scripts/train_streaming.py --model-size medium --max-steps 100000"
echo ""
echo "4) Large 模型 (27B) - DeepSpeed + 流式 (4卡)"
echo "   显存需求: 4x 96GB + CPU Offload"
echo "   命令: deepspeed --num_gpus=4 scripts/train_streaming.py --model-size large --use-deepspeed"
echo ""
echo "5) FineWeb 数据集流式训练示例"
echo "   命令: python scripts/train_streaming.py --model-size small --dataset fineweb --max-steps 50000"
echo ""
echo "6) 运行基准测试"
echo "   命令: python scripts/run_benchmarks.py --model-size medium --benchmarks all"
echo ""
echo "7) 验证配置"
echo "   命令: python scripts/train_h20.py --model-size medium"
echo ""
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    python scripts/train_h20.py --model-size medium
fi
