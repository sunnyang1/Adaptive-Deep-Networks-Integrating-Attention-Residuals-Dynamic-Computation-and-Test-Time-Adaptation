...
echo ""
echo "传统训练 (需下载数据集到本地):"
echo "  python scripts/train_model.py --model-size medium --batch-size 2 --output-dir $CHECKPOINT_DIR"
echo ""
echo "流式加载训练 (推荐! 零本地存储):"
echo "  python scripts/train_streaming.py --model-size medium --max-steps 10000"
echo ""
echo "单机多卡训练 (4x H20) + 流式加载:"
echo "  torchrun --nproc_per_node=4 scripts/train_streaming.py --model-size medium --max-steps 100000"
echo ""
echo "DeepSpeed + 流式加载 (Large 27B):"
echo "  deepspeed --num_gpus=4 scripts/train_streaming.py --model-size large --use-deepspeed"
echo ""
