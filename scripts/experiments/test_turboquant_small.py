...
class TurboQuantTester:
    ...
    def test_1_compression_ratio(self):
        ...
        print(f"  Compressed: 16 + {dim-1}×{self.angle_bits} = {results['polar_quant']['compressed_bits']} bits")
    ...
    def generate_report(self):
        ...
        if 'polar_quant_accuracy' in self.results:
            ...
            for r in pq['by_bit_width']:
                report.append(f"{r['angle_bits']}-bit: {r['compression_ratio']:.1f}× compression, "
        ...
        if 'qjl_accuracy' in self.results:
            ...
            for r in qjl['by_proj_dim']:
                report.append(f"proj_dim={r['proj_dim']}: {r['compression_ratio']:.1f}× compression, "
                             f"{r['relative_error']*100:.2f}% error, "
                             f"bias={r['bias']:.4f}")
