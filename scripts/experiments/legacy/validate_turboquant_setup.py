...
def check_polar_pseudo_queries():
    ...
    try:
        ...
        manager = create_pseudo_query_manager(
            num_layers=4, dim=64, num_blocks=2, use_polar=True, enable_qttt=True
        )
...
def check_polar_qttt():
    ...
    try:
        ...
        config = PolarQTTTConfig(
            num_steps=4, learning_rate=0.01, adapt_magnitude=False, adapt_direction=True
        )

        qttt = PolarQTTT(config, hidden_dim=128, num_heads=4)

        # Test cost calculation
        cost = qttt.compute_effective_cost(batch_size=1, seq_len=1000, use_turboquant=True)
...
def check_depth_priority_gating():
    ...
    try:
        ...
        controller = create_depth_priority_controller(
            target_rate=0.3, max_qttt_steps=32, turboquant_enabled=True
        )

        # Test allocation
        should_adapt, steps, think, threshold = controller.decide(reconstruction_loss=5.0)
