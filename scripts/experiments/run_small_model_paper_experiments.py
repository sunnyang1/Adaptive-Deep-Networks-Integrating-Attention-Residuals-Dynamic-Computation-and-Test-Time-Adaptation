...
class SmallModelExperiments:
    ...
    def experiment_2_needle_haystack(self, context_lengths: List[int] = None) -> Dict:
        ...
        results = {"context_lengths": context_lengths, "scores": []}
        ...
        for ctx_len in context_lengths:
            ...
            results["scores"].append(
                {"context_length": ctx_len, "score": score, "tested_length": seq_len}
            )
    ...
    def experiment_3_flop_analysis(self) -> Dict:
        ...
        print(f"\nFLOP Equivalence:")
        print(
            f"  T_think ≈ 2 * {max_qttt_steps} * {qttt_span} = {equivalent_thinking_tokens} tokens"
        )
    ...
    def experiment_4_model_components(self) -> Dict:
        ...
        for comp, count in sorted(component_params.items(), key=lambda x: -x[1]):
            ...
            results["components"][comp] = {"params": count, "percentage": pct, "human": human}
        ...
        print(
            f"  Memory reduction: O(Ld) -> O(Nd) = O({self.config.num_layers}d) -> O({self.config.num_blocks}d)"
        )
    ...
    def generate_report(self):
        ...
        if "flop_analysis" in self.results:
            ...
            report.append(
                f"qTTT equivalent: {fa['qttt']['equivalent_thinking_tokens']} thinking tokens"
            )
        ...
        if "inference_performance" in self.results:
            ...
            for t in ip["latency_tests"]:
                report.append(
                    f"{t['seq_len']:4d} tokens: {t['avg_time_ms']:7.1f}ms ({t['throughput_tok_per_sec']:6.1f} tok/s)"
                )
