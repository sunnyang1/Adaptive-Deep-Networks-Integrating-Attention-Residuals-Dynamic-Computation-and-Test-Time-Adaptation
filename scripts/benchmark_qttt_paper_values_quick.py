...
def test_quick_comparison():
    ...
    with torch.no_grad():
        output_paper = model.generate(
            input_ids, max_new_tokens=max_new_tokens, use_qttt=True, 
            qttt_config=paper_config, use_attnres=True
        )
    ...
    with torch.no_grad():
        output_opt = model.generate(
            input_ids, max_new_tokens=max_new_tokens, use_qttt=True,
            qttt_config=opt_config, use_attnres=True
        )
    t_opt = time.time() - t0
    ...
    with torch.no_grad():
        output_short = model.generate(
            input_ids, max_new_tokens=max_new_tokens, use_qttt=True,
            qttt_config=short_config, use_attnres=True
        )
