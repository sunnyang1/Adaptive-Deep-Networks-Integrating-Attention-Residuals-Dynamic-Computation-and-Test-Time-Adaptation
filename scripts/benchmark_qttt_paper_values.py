...
def test_paper_vs_optimized():
    ...
    results['baseline'] = {
        'time': t_baseline,
        'tokens_per_sec': max_new_tokens/t_baseline
    }
    ...
    match_count = sum(
        a == b for a, b in 
        zip(output_baseline[0, prompt_len:].tolist(), output_paper[0, prompt_len:].tolist())
    )
    ...
    match_count_opt = sum(
        a == b for a, b in 
        zip(output_baseline[0, prompt_len:].tolist(), output_optimized[0, prompt_len:].tolist())
    )
    ...
    match_count_short = sum(
        a == b for a, b in 
        zip(output_baseline[0, prompt_len:].tolist(), output_short[0, prompt_len:].tolist())
    )
    ...
    print(f"{'Paper (10 steps, 0.01)':<25} {t_paper:<12.3f} {t_paper/t_baseline:<12.1f} {match_count}/{max_new_tokens}")
    print(f"{'Optimized (2 steps, 0.02)':<25} {t_optimized:<12.3f} {t_optimized/t_baseline:<12.1f} {match_count_opt}/{max_new_tokens}")
    print(f"{'Paper Short (2 steps, 0.01)':<25} {t_short:<12.3f} {t_short/t_baseline:<12.1f} {match_count_short}/{max_new_tokens}")
