...
from src.qttt.polar_adaptation import (
    SphericalSGD,
    PolarQueryAdapter as QTTTAdapter,
    PolarQTTT,
...
class TestPolarPseudoQueryManager:
    ...
    def test_parameter_count_reduction(self):
        ...
        # Cartesian mode
        cart_manager = PolarPseudoQueryManager(num_layers, dim, num_blocks, use_polar=False)
        cart_params = cart_manager.get_parameter_count()

        # Polar mode with qTTT
        polar_manager = PolarPseudoQueryManager(num_layers, dim, num_blocks, use_polar=True)
...
class TestPolarQTTTAdapter:
    ...
    def test_spherical_update(self):
        ...
        config = PolarQTTTConfig(adapt_magnitude=False, use_spherical_sgd=True, learning_rate=0.1)
...
class TestPolarQTTT:
    ...
    def test_effective_cost_calculation(self):
        ...
        cost_without = qttt.compute_effective_cost(batch_size=1, seq_len=1000, use_turboquant=False)
...
class TestDepthPriorityController:
    ...
    def test_strict_depth_priority(self):
        ...
        controller = DepthPriorityController(
            max_qttt_steps=32, think_tokens_if_forced=0, turboquant_enabled=True
        )
...
class TestDepthPriorityGating:
    ...
    def test_decision_structure(self):
        ...
        controller = DepthPriorityGatingController(
            threshold_calibrator=threshold, max_qttt_steps=32, turboquant_enabled=True
        )
    ...
    def test_allocation_report(self):
        ...
        threshold = TargetRateThreshold(target_rate=0.3)
        controller = DepthPriorityGatingController(
            threshold_calibrator=threshold, turboquant_enabled=True
        )
    ...
    def test_policy_comparison(self):
        ...
        assert "8x" in comparison["turboquant_policy"]["savings_vs_standard"] or "8.0" in str(
            comparison["turboquant_policy"]["savings_vs_standard"]
        )
...
class TestAdaptiveThreshold:
    ...
    def test_depth_adjustment(self):
        ...
        threshold = AdaptiveThresholdWithDepthPriority(target_rate=0.3, max_depth_steps=32)
...
class TestIntegration:
    ...
    def test_factory_functions(self):
        ...
        # Create controller
        controller = create_depth_priority_controller(target_rate=0.3, turboquant_enabled=True)
