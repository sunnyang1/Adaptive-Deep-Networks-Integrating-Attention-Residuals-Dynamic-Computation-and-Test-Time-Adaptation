...
def test_attnres_gradient_flow():
    ...
    models_to_test = [
        (
            "Standard",
            lambda: StandardResidualModel(config.d_model, config.n_layers).to(config.device),
        ),
        (
            "BlockAttnRes",
            lambda: BlockAttnResModel(
                config.d_model, config.n_layers, config.n_layers // config.n_blocks
            ).to(config.device),
        ),
        (
            "FullAttnRes",
            lambda: FullAttnResModel(config.d_model, config.n_layers).to(config.device),
        ),
    ]
    ...
    for name, model_fn in models_to_test:
        ...
        print(
            f"  {name:15s} CV(∇) = {cv:.3f} (μ={grad_norms.mean():.4f}, σ={grad_norms.std():.4f})"
        )
...
def test_rabitq_memory_stats():
    ...
    for bits in [16, 3, 2, 1]:  # 16=FP16 baseline
        if bits == 16:
            # FP16 baseline
            bytes_per_element = 2
            storage_gb = (seq_len * num_layers * num_heads * head_dim * bytes_per_element) / (
                1024**3
            )
            name = "FP16 (baseline)"
        else:
        ...
        results.append({"name": name, "bits": bits, "storage_gb": storage_gb})

        print(f"  {name:18s}: {storage_gb:.2f} GB")

    # Verify storage numbers (single KV tensor; paper table may count K+V separately)
    assert (
        abs(results[0]["storage_gb"] - 20.0) < 1.0
    ), "FP16 should be ~20GB for one KV matrix at these dims"
...
def test_qttt_polar_adaptation():
    ...
    print(
        f"  Paper claim: 50% reduction ✅"
        if abs(reduction - 0.5) < 0.1
        else "  ⚠️ Check parameter counting"
    )

    return {"loss_history": loss_history, "param_reduction": reduction}
...
def test_qttt_adaptive_config():
    ...
    for seq_len, category in test_lengths:
        for mode in ["fast", "balanced", "quality"]:
            ...
            print(
                f"  {category:20s} | {mode:8s} | steps={config_dict['num_steps']:2d} | lr={config_dict['learning_rate']:.4f}"
            )

    # Verify: longer sequences get more steps (compare same mode across lengths)
    for mode in ["fast", "balanced", "quality"]:
        s = next(
            r["num_steps"] for r in results if r["category"] == "Short (< 4K)" and r["mode"] == mode
        )
        m = next(
            r["num_steps"]
            for r in results
            if r["category"] == "Medium (4K-32K)" and r["mode"] == mode
        )
        l = next(
            r["num_steps"] for r in results if r["category"] == "Long (> 32K)" and r["mode"] == mode
        )
...
def test_full_pipeline():
    ...
    with torch.no_grad():
        logits_frozen = model.forward_with_frozen_kv(input_ids, kv_caches, use_attnres=True)
...
def test_performance_comparison():
    ...
    for name, kwargs in configs:
        ...
        results.append({"config": name, "avg_time_ms": avg_time, "std_time_ms": std_time})
