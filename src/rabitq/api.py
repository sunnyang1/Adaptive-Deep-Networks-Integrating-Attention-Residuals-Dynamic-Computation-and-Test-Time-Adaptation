...
from .quantizer import (
    QuantizedVector, RabitqConfig, quantize_vector, reconstruct_vector,
    compute_const_scaling_factor, quantize_scalar
)
...
class RaBitQ:
    ...
    def __init__(self, config: Optional[RaBitQConfig] = None,
                 total_bits: int = 1, head_dim: int = 64, device: str = 'cpu'):
        if config is None:
        ...
        if config.use_rotation:
            if config.rotator_type == 'fht':
                self.rotator = FhtKacRotator(config.head_dim, seed=config.rotator_seed, device=config.device)
    ...
    def _compress_tensor(self, rotated: torch.Tensor, centroid: torch.Tensor,
                         original_shape: Tuple[int, ...]) -> CompressedKV:
        ...
        for i in range(flat.shape[0]):
            ...
            qvs.append(qv)
        return CompressedKV(quantized_vectors=qvs, original_shape=original_shape,
                            centroid=centroid.clone())

    def _decompress_tensor(self, ck: CompressedKV) -> torch.Tensor:
        """Decompress a CompressedKV."""
        flat = torch.stack([
            reconstruct_vector(ck.centroid, qv)
