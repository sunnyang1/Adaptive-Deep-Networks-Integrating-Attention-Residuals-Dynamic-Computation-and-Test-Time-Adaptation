...
def preprocess_query(query: torch.Tensor, centroid: torch.Tensor,
...
def make_full_single_query(rotated_query: torch.Tensor, centroid: torch.Tensor,
                           metric_type: str = 'ip',
                           total_bits: int = 1) -> FullSingleQuery:
...
def full_est_dist(ip: float, f_add: float, f_rescale: float,
                  query: FullSingleQuery) -> Tuple[float, float]:
    """
...
def split_single_estdist(f_add: float, f_rescale: float, f_error: float,
                         bin_code: torch.Tensor,
                         query: SplitSingleQuery) -> Tuple[float, float, float]:
    """
...
def split_single_fulldist(f_add_ex: float, f_rescale_ex: float, f_error: float,
                          ex_code: torch.Tensor, ex_bits: int,
                          ip_x0_qr: float,
                          query: FullSingleQuery) -> Tuple[float, float]:
    """
    ...
    est_dist = f_add_ex + query.g_add + f_rescale_ex * (
        (1 << ex_bits) * ip_x0_qr + ip_ex + query.kbxsumq
    )
    low_dist = est_dist - (f_error * query.g_error / (1 << ex_bits))
    return est_dist, low_dist


def split_distance_boosting(f_add_ex: float, f_rescale_ex: float,
                            ex_code: torch.Tensor, ex_bits: int,
                            ip_x0_qr: float,
                            query: FullSingleQuery) -> float:
    """
...
def estimate_inner_product(query: torch.Tensor, centroid: torch.Tensor,
                           qv, query_bits: int = 8) -> float:
    """
