...
class RaBitQCache:
    ...
    def __init__(self, config: Optional[CacheConfig] = None,
                 total_bits: int = 1, head_dim: int = 64,
                 rotator: Optional[Rotator] = None,
                 residual_window: int = 128, device: str = 'cpu'):
        if config is None:
            config = CacheConfig(
    ...
    def get_kv(self, layer_idx: int, dtype: torch.dtype = torch.float32) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _compress_tokens(self, states: torch.Tensor, centroid: torch.Tensor) -> List[QuantizedVector]:
        ...
        return [quantize_vector(flat[i], centroid, self.rabitq_config) for i in range(flat.shape[0])]

    def _decompress_tokens(self, qvs: List[QuantizedVector], centroid: torch.Tensor,
                           B: int, H: int, D: int) -> torch.Tensor:
