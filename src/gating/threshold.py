...
class EMAThreshold(DynamicThreshold):
    ...
    def __init__(
        self, initial_threshold: float = 2.0, beta: float = 0.99, percentile: float = 70.0
    ):
...
class GatingController:
    ...
    def decide(
        self, reconstruction_loss: float, input_features: Optional[torch.Tensor] = None
        ...
        if should_adapt:
            ...
            num_steps = min(self.max_steps, max(self.min_steps, int(self.min_steps * (1 + excess))))
        else:
            num_steps = 0
    ...
    def get_oracle_recovery(self, oracle_labels: list) -> float:
        ...
        matches = sum(1 for i in range(n) if self.decision_history[i]["adapt"] == oracle_labels[i])
