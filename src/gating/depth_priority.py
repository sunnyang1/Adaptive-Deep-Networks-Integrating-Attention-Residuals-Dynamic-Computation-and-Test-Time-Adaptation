...
class DepthPriorityGatingController:
    ...
    def decide(
        ...
        if should_adapt:
            ...
            num_steps = min(
                self.max_qttt_steps,
                max(self.min_qttt_steps, 
                    int(self.min_qttt_steps * (1 + excess * 2)))
            )
    ...
    def update(self, loss_value: float) -> float:
        ...
        if len(self.recent_decisions) >= 50:
            ...
            if current_rate > self.target_rate + self.rate_tolerance:
                ...
                self.recommended_depth_steps = max(
                    4, self.recommended_depth_steps - 2
                )
...
def create_depth_priority_controller(
    ...
    if use_adaptive_threshold:
        threshold = AdaptiveThresholdWithDepthPriority(
            ...
            max_depth_steps=max_qttt_steps
        )
    else:
        threshold = TargetRateThreshold(
            target_rate=target_rate
        )
