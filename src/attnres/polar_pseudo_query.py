...
class PolarPseudoQueryManager(nn.Module):
    ...
    def __init__(
        ...
        if use_polar:
            ...
            with torch.no_grad():
                self.directions.data = F.normalize(
                    self.directions.data, dim=-1
                )
        else:
        ...
        # Statistics tracking
        self.register_buffer('attention_history', 
                           torch.zeros(100, num_layers, num_blocks + 1))
        self.history_ptr = 0
    ...
    def get_parameter_count(self) -> Dict[str, int]:
        ...
        if self.use_polar:
            ...
            trainable = (
                (self.magnitudes.numel() if self.magnitudes.requires_grad else 0) +
...
class PseudoQueryPolarAdapter:
    ...
    def adapt(
        ...
        if v_norm > 1e-8:
            ...
            new_direction = (direction * torch.cos(v_norm) + 
