...
class MNNTurboQuantCompressor:
    ...
    def __init__(self, config: MNNTurboQuantConfig, head_dim: int = 128, device: str = "cpu"):
        self.config = config
    ...
    def get_memory_stats(
        self, seq_len: int, batch_size: int = 1, num_heads: int = 32
    ) -> Dict[str, float]:
...
def create_mnn_turboquant(
    attention_mode: int = 8, head_dim: int = 128, device: str = "cpu"
    ...
    """
    config = MNNTurboQuantConfig(attention_mode=attention_mode, use_lloyd_max=True)
