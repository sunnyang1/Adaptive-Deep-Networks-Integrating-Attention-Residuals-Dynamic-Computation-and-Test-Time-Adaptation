...
class TurboQuantPipeline(nn.Module):
    ...
    def __init__(self, dim: int, config: TurboQuantConfig, device="cpu"):
        super().__init__()
        self.dim = dim
        self.config = config
        self.device = device
    ...
    def compress_kv_cache(
        ...
        bytes_compressed = sum(
            v.numel() * (1 if v.dtype == torch.int8 else 2) for v in compressed.values()
        )

        self.compression_stats["bytes_original"] += bytes_original
        self.compression_stats["bytes_compressed"] += bytes_compressed
        self.compression_stats["compression_ratio"] = self.compression_stats[
            "bytes_original"
    ...
    def decompress_kv_cache(
        ...
        keys = self.polar_quant.decompress(
            compressed["keys_r"].reshape(-1, 1), compressed["keys_theta"].reshape(-1, d - 1)
        ).reshape(B, H, T, d)

        # Decompress values
        values = self.polar_quant.decompress(
            compressed["values_r"].reshape(-1, 1), compressed["values_theta"].reshape(-1, d - 1)
        ).reshape(B, H, T, d)
    ...
    def compute_attention_with_compressed_kv(
        ...
        keys_polar = self.polar_quant.decompress(
            compressed_kv["keys_r"].reshape(B, H, T, 1), compressed_kv["keys_theta"]
        )  # [B, H, T, d]
        ...
        if values is None:
            values = self.polar_quant.decompress(
                compressed_kv["values_r"].reshape(B, H, T, 1), compressed_kv["values_theta"]
            )
...
class TurboQuantLinear(nn.Module):
    ...
    def __init__(
        self, in_features: int, out_features: int, config: TurboQuantConfig, bias: bool = True
    ):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._compressed_weight is None:
            ...
            with torch.no_grad():
                self._compressed_weight = self.turbo.compress_vector(self.weight)
