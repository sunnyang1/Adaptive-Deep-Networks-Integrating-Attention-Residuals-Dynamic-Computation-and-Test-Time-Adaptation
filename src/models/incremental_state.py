...
class IncrementalState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        """
        return {
            'kv_caches': [(cache.keys.cpu().numpy(), cache.values.cpu().numpy()) 
    ...
    def get_memory_usage(self) -> Dict[str, int]:
        ...
        block_memory = sum(
            ...
            for b in self.block_representations
        )
...
def validate_state(state: IncrementalState, raise_on_error: bool = True) -> bool:
    ...
    if len(state.block_representations) > state.num_blocks:
        errors.append(f"Block rep count ({len(state.block_representations)}) > num_blocks ({state.num_blocks})")
