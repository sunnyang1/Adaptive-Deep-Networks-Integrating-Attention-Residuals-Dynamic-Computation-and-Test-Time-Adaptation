    ...
    def forward(
        ...
        if use_attnres and attnres_module is not None:
            h_attn, _ = attnres_module(
                block_representations, partial_block, use_attn=True, use_mlp=False
            )
        else:
        ...
        if use_attnres and attnres_module is not None:
            _, h_mlp = attnres_module(
                block_representations, partial_block, use_attn=False, use_mlp=True
            )
        else:
            h_mlp = partial_block
...
class AdaptiveTransformer(nn.Module):
    ...
    def __init__(self, config: ModelConfig):
        ...
        # Transformer layers
        self.layers = nn.ModuleList([AdaptiveLayer(config, i) for i in range(config.num_layers)])
    ...
    def forward(
        ...
        if use_rabitq and rabitq_caches is not None and len(self._rabitq_kv_cache) == 0:
            ...
            rabitq_kv_caches = self._build_rabitq_kv_cache(
                ...
                rabitq_caches,
                use_attnres,
                attnres_modules=self.attnres_modules,
                input_ids=input_ids,
            )
        ...
        for layer_idx, (layer, attnres) in enumerate(zip(self.layers, self.attnres_modules)):
            ...
            rq_cache = (
                rabitq_caches[layer_idx] if (use_rabitq and rabitq_caches is not None) else None
            )
    ...
    def generate(
        ...
        for step in range(max_new_tokens):
            ...
            if ponder_gate is not None and step > 0 and next_token_logits is not None:
                ...
                if isinstance(adapt_decision, torch.Tensor):
                    ...
                    should_run_qttt = bool(adapt_decision)
            ...
            logits = self.forward(
                ...
                adapted_query_layer_idx=(
                    self.config.num_layers - 1 if adapted_query is not None else None
                ),
        ...
        if ponder_gate is not None and qttt_trigger_count > 0:
            print(
                f"[Ponder Gate] qTTT triggered {qttt_trigger_count}/{max_new_tokens} times "
        ...
        if adaptive_qttt_mode is not None and use_qttt:
            print(
                f"[Adaptive qTTT] Mode: {adaptive_qttt_mode}, "
    ...
    def count_attnsres_parameters(self) -> int:
        ...
        return sum(
            p.numel() for n, p in self.named_parameters() if "pseudo_query" in n or "attnres" in n
        )


def create_adaptive_transformer(config_name: str = "medium", **kwargs) -> AdaptiveTransformer:
