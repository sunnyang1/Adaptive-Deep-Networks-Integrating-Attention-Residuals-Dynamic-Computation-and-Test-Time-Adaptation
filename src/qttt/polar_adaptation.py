...
class SphericalSGD:
    ...
    def step(self, point: torch.Tensor, gradient: torch.Tensor) -> torch.Tensor:
        """
        ...
        if v_norm > 1e-8:
            new_point = point * torch.cos(v_norm) + (self.velocity / v_norm) * torch.sin(v_norm)
        else:
            new_point = point
...
class QueryAdaptationPolarAdapter:
    ...
    def __init__(self, magnitude: torch.Tensor, direction: torch.Tensor, config: PolarQTTTConfig):
        """
        Args:
            magnitude: Scalar r
        ...
        if config.use_spherical_sgd:
            self.spherical_opt = SphericalSGD(learning_rate=config.learning_rate, momentum=0.9)
        else:
...
class PolarQTTT(nn.Module):
    ...
    def __init__(self, config: PolarQTTTConfig, hidden_dim: int, num_heads: int = 32):
        super().__init__()
        self.config = config
        ...
        # Statistics tracking
        self.adaptation_stats = {"num_calls": 0, "total_steps": 0, "avg_loss_reduction": 0.0}
    ...
    def adapt_pseudo_query(
        ...
        for step in range(self.config.num_steps):
            ...
            if (
                ...
                and len(loss_history) > 1
                and abs(loss_history[-1] - loss_history[-2]) < self.config.early_stop_threshold
            ):
                break
    ...
    def adapt_query_projection(
        ...
        for step in range(self.config.num_steps):
            ...
            if model is not None and input_ids is not None and kv_caches is not None:
                ...
                loss = self._compute_adaptation_loss(
                    ...
                    (
                        ...
                        else torch.arange(T, device=queries.device)
                    ),
                    target_token_ids,
            ...
            with torch.no_grad():
                ...
                new_u = u_norm.clone()
                if mask.any():
                    new_u[mask] = u_norm[mask] * torch.cos(step_norm[mask]) + (
                        step_vec[mask] / (step_norm[mask] + 1e-8)
                    ) * torch.sin(step_norm[mask])
    ...
    def _compute_adaptation_loss(
        ...
        if self.config.loss_type == "cross_entropy":
            ...
            raise ValueError(
                f"Unknown loss_type: {self.config.loss_type}. "
