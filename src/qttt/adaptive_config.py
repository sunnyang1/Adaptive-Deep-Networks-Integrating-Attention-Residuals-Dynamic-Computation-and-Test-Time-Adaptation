...
class AdaptiveQTTTConfig:
    ...
    def get_lr_for_gradient(self, grad_norm: float) -> float:
        ...
        """
        return compute_adaptive_lr(grad_norm=grad_norm, base_lr=self.base_lr, min_lr=self.min_lr)
...
def compute_adaptive_lr(
    grad_norm: float, base_lr: float = 0.01, min_lr: float = 0.001, target_grad: float = 0.1
) -> float:
    """
    Compute adaptive learning rate based on gradient magnitude.
...
def create_adaptive_config(mode: str = "balanced") -> AdaptiveQTTTConfig:
    """
